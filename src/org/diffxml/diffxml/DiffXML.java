...
import org.diffxml.diffxml.fmes.*;
import org.diffxml.diffxml.xmdiff.*;
...
public class DiffXML
{
	...
	while (i < args.length && args[i].startsWith("-")) 
	    ...
	    /* Normalize multiple dashes
	       I don't understand point in differentiating between 1 and 2 dashes
		...
		{
		Fmes.ENTITIES=false;
		System.err.println("STERN WARNING: Removing entities may lead to incorrect or misleading results");
		}
		...
		if (num<0)
		    {
		    System.err.println("Sibling Context must positive integer > 0");
		    System.exit(2);
		    }
		...
		if (num<0)
		    {
		    System.err.println("Parent Context must positive integer > 0");
		    System.exit(2);
		    }
		...
		if (num<0)
		    {
		    System.err.println("Parent Sibling Context must positive integer > 0");
		    System.exit(2);
		    }
		...
		for (j = 1; j < arg.length(); j++) {
			...
			default:
			    System.err.println("diffxml: illegal option " + flag);
			    printUsage();
			    break;
		    }
		}
    ...
    private static void initParser(DOMParser parser)
	{
	try 
	    {
	...
	if (differ)
	    ...
	    if (Fmes.BRIEF)
		{
		System.out.println("XML documents " + _file1 + " and " + _file2 + " differ");
		}
