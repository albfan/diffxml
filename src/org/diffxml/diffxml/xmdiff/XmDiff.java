/*
Class implements xmdiff algorithm
...
public class XmDiff extends Diff
{
	...
	if(eventType == doc1.START_TAG) 
		...
		PrintPath(path, fA, depth);	
            ...
            eventType = doc1.next();
        } while (eventType != doc1.END_DOCUMENT);
	...
	do {
		...
		//Do Path Stuff
            ...
            eventType = doc2.next();
           } while (eventType != doc2.END_DOCUMENT);
	...
	*/
	...
	do {
	   ...
	   else if (eventType1==doc1.TEXT)
		i_node.set( Node.TEXT, doc1.getText(), (doc1.getDepth()+1) );
			
	   //Get next tag 
	   if (eventType1!=doc1.END_DOCUMENT)
	   	eventType1_next=doc1.next();

	   while (eventType1_next != doc1.START_TAG && eventType1_next != doc1.TEXT && eventType1_next != doc1.END_DOCUMENT)
		{
	   ...
	   if (eventType1==doc1.START_TAG || eventType1==doc1.TEXT)
                {
		i++;
		j=0;
	   	do
			...
			while (eventType2 != doc2.START_TAG && eventType2 != doc2.TEXT && eventType2 != doc2.END_DOCUMENT)
                		eventType2=doc2.next();

			if (eventType2==doc2.START_TAG || eventType2==doc2.TEXT)	
				{
				j++;
				...
				if (eventType2==doc2.TEXT)
					j_depth++;
				if ( (eventType2==eventType1) && (i_node.depth==j_depth) )
					{
					if (eventType2==doc2.START_TAG)
						{
			...
			//eventType2 = doc2.next();	
			} while (eventType2 != doc2.END_DOCUMENT);		
		}
	   //eventType1 = doc1.next();
	   } while (eventType1 != doc1.END_DOCUMENT);
	}
